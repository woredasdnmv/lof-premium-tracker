...
class HistoryDB:
    ...
    def save_snapshot(self, funds: Dict[str, dict]):
        """
        保存当前时刻的溢价率快照
        funds: { code: { "premium_rate": float, "price": float, "nav": float, ... }, ... }
        """
        ...
        for code, fund in funds.items():
            ...
            if premium is None:
                continue
            rows.append((today, code, premium, price, nav))
        ...
        try:
            conn.executemany(
                "INSERT OR REPLACE INTO premium_snapshots (date, code, premium_rate, price, nav) VALUES (?, ?, ?, ?, ?)",
                rows
            )
            conn.commit()
    ...
    def get_avg_premium_3d(self, code: str) -> Optional[float]:
        ...
        rows = conn.execute(
            """
            SELECT date, premium_rate FROM premium_snapshots
        ...
        total = sum(r["premium_rate"] for r in rows)
    ...
    def get_all_avg_premium_3d(self) -> Dict[str, Optional[float]]:
        ...
        rows = conn.execute(
            f"""
            SELECT code, AVG(premium_rate) as avg_prem
        ...
        return {r["code"]: round(r["avg_prem"], 3) for r in rows}
    ...
    def get_history(self, code: str = None, days: int = 7) -> list:
        ...
        if code:
            rows = conn.execute(
                """
                SELECT date, code, premium_rate, price, nav
                FROM premium_snapshots
                WHERE code = ? AND date >= ?
                ORDER BY date DESC
                """,
